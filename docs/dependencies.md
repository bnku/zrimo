...
Qualification date: 2026-07-17. Runtime versions are exact pins in `package-lock.json` and `Cargo.lock`; build-tool versions are pinned in `package.json` and `rust-toolchain.toml`.
...
| `office_oxide`            |                         `0.1.6` | MIT OR Apache-2.0 | CFB reader, normalized IR/DOCX writer, XLS/PPT conversion and diagnostic DOC text extraction                             | XLS/PPT convert in a browser worker; DOC uses the separate project-owned parser and only reuses permissive container/IR/writer code                                         |
| `pdfjs-dist`              |                       `6.1.200` | Apache-2.0        | Lazy browser display API, dedicated module worker, packaged CMaps/standard fonts/WASM/ICC assets; scripting/XFA disabled | Type 1, CFF/CID, complex TrueType, Arabic CID, non-embedded JIS and Base-14 corpus renders without rectangle fallback; text layer, cancellation and local-only network pass |
...
| `zip`                     |                         `8.6.0` | MIT               | In-memory patching of source-backed DOC comment parts after shared DOCX serialization                                    | WASM check and public point-comment structural qualification pass; no filesystem path is used                                                                               |
...
The selected `office_oxide` release does not expose bytes-out in its stock JavaScript binding, but the Rust API publicly exposes `create_from_ir_to_writer`. `crates/legacy-office-wasm` is therefore a small project-owned binding. Its generated web binding is loaded only inside `legacy-converter-worker.js`; DOC/XLS/PPT conversion uses bytes-in/bytes-out transferables and no filesystem or server. DOC is routed through `crates/legacy-doc`, so the stock text-to-heading heuristic cannot be reached by the render path.

The old `pdf_oxide` rendering crate, WASM binding, PNG bridge, worker and page
cache are removed from the workspace and npm artifact. A parser-only
`pdf_oxide` dependency remains solely in the separate cargo-fuzz workspace; it
is not built, linked, copied or shipped by the viewer.
...
The current 2026-07-17 runtime baseline passes for 5 npm packages, 60 Cargo
packages, and 14 verified font assets. Vitest/Vite was removed during
qualification because Vite's development tree introduced MPL-2.0
`lightningcss`; unit tests now use the zero-dependency Node test runner.
`deny.toml` mirrors the Cargo allowlist for environments with `cargo-deny`
installed.
