...
| `office_oxide`            |                         `0.1.6` | MIT OR Apache-2.0 | Default features disabled; XLS/PPT conversion plus diagnostic DOC text extraction in our WASM crate                      | XLS/PPT fixtures convert in a browser module worker; DOC conversion is fail-closed because upstream exposes no formatting/section/table model                               |
...
The selected `office_oxide` release does not expose bytes-out in its stock JavaScript binding, but the Rust API publicly exposes `create_from_ir_to_writer`. `crates/legacy-office-wasm` is therefore a small project-owned binding. Its generated web binding is loaded only inside `legacy-converter-worker.js`; XLS/PPT conversion uses bytes-in/bytes-out transferables and no filesystem or server. DOC's FIB/CLX reader remains useful for an explicitly named diagnostic text extractor, while the conversion binding refuses DOC before `to_ir()` can fabricate headings.
